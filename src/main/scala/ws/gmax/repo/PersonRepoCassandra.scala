...
class PersonRepoCassandra(session: Session, isAsync: Boolean)(implicit ec: ExecutionContext)
  extends Java2ScalaFuture(isAsync) with PersonRepo {
...
object PersonRepoCassandra {
  def apply(session: Session, isAsync: Boolean = true)
           (implicit ec: ExecutionContext): PersonRepoCassandra =
    new PersonRepoCassandra(session, isAsync)
}